...
CGV 영화 예매 서비스를 간소화하여 구현한 클론 프로젝트입니다.
...
```bash
git clone https://github.com/hy-cj-ai-cloud-bootcamp/cgv-clone-template.git
cd cgv-clone-template
cp .env.example .env
...
1. 원본 템플릿 레포에 접속합니다.
   - https://github.com/hy-cj-ai-cloud-bootcamp/cgv-clone-template
...
upstream  https://github.com/hy-cj-ai-cloud-bootcamp/cgv-clone-template.git
```
...
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | /api/movies | 영화 목록 (?status=showing/upcoming) |
| GET | /api/movies/{id} | 영화 상세 |

### 상영 스케줄
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | /api/schedules | 스케줄 조회 (?date=&movie_id=) |
| GET | /api/schedules/{id} | 스케줄 상세 (좌석 정보 포함) |

### 예매 (로그인 필요)
| Method | Endpoint | 설명 |
|--------|----------|------|
| POST | /api/bookings | 예매하기 |
| GET | /api/bookings | 예매 내역 |
...
> 중요: 현재 프론트 코드는 `/api/movies`, `/api/auth/login`처럼 상대 경로로 API를 호출합니다. 따라서 CloudFront에서 `/api/*` 요청을 ALB로 보내야 합니다. S3만 연결하면 API 요청이 실패합니다.
...
### 아키텍처
```
사용자 질문 → Streamlit → FastAPI → DB에서 영화/스케줄 데이터 조회
                                         ↓
                                  영화 정보를 프롬프트에 포함
                                         ↓
...
### 구현 방식
- 데이터: RDS에 저장된 영화/상영 데이터를 DB에서 직접 조회
- 이미지: S3에 영화 포스터 업로드 후 DB의 poster_url 컬럼 업데이트
...
- 실제 CGV 서비스와 무관하며, 상업적 목적이 아닌 클라우드/AI 실습 목적으로만 사용합니다.
